...
// 라우트: 주문 제출
// 라우트: 주문 제출
// - 매장 PC(윈도우)에서는 바로 프린터 출력
// - Railway(리눅스)에서는 주문 저장만 → 매장 PC의 print-client.js가 가져가서 출력
app.post('/api/orders', (req, res) => {
    try {
        ...
        console.log('----------------------------------------');
        console.log(`합계: ${order.total.toLocaleString()}원`);
        console.log('========================================\n');
...
// 라우트: 주문 목록 조회 (관리자 + print-client.js가 사용)
app.get('/api/orders', (req, res) => {
    try {
        ...
        for (const file of files) {
            try {
                orders.push(JSON.parse(fs.readFileSync(path.join(ordersDir, file), 'utf-8')));
            } catch (e) {
                // 깨진 파일은 건너뜀
            }
        }
...
// 함수: 프린터로 출력 (윈도우 PC에서만 실행)
// 실제 인쇄는 print-receipt.ps1 이 담당 (글자 크기는 그 파일 맨 위에서 조절)
function printOrder(order) {
    if (process.platform !== 'win32') {
        // Railway(리눅스)에는 프린터가 없으므로 출력하지 않음
        return;
    }
    ...
    const printCommand = `powershell -NoProfile -ExecutionPolicy Bypass -File "${scriptPath}" -Path "${printFile}"`;
...
// 함수: 프린터 출력 내용 생성
// [TITLE] [TABLE] [TIME] [MENU] 표시는 print-receipt.ps1 이 글자 크기를 정하는 데 사용
function generatePrintContent(order) {
    const clean = (s) => String(s ?? '').replace(/[\t\r\n]+/g, ' ').trim();

    let content = '';
    content += '[TITLE]금별맥주 야장 주문서\n';
    content += `[TABLE]${clean(order.tableNumber)}번 테이블\n`;
    content += `[TIME]${clean(order.timestamp)}\n`;
    content += '\n';

    (order.items || []).forEach(item => {
        const qty = item.qty || item.quantity || 1;
        content += `[MENU]${clean(item.name)}\t${qty}개\n`;
    });
...
app.listen(PORT, () => {
    console.log('\n🚀 금별맥주 QR 주문 시스템 시작됨!');
    console.log('📱 손님 주문: https://andone-order.com/?table=테이블번호');
    console.log('🖨️ QR 인쇄: https://andone-order.com/qr-print.html');
    console.log(`\n매장 내부 접속: http://192.168.0.99:${PORT}/?table=테이블번호`);
    console.log(`관리자: http://192.168.0.99:${PORT}/admin.html\n`);
});
